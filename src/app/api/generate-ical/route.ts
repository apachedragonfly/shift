...
export async function GET(request: NextRequest) {
	try {
		...
		const { data: shiftsData, error: shiftsError } = await supabaseWithAuth
			.from('shifts')
			.select('*')
			.eq('user_id', user.id)
			.order('date', { ascending: true })

		console.log('Shifts query result:', { shifts: shiftsData, shiftsError, count: shiftsData?.length })
		...
		const events = (shiftsData as Shift[]).map((shift) => convertShiftToEvent(shift))
